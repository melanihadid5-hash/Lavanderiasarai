...
export async function GET(req) {
  ...
  try {
    ...
    // Tampoco devuelve el apellido: el personal lo pide de viva voz al
    // entregar para confirmar que la bolsa se da a su dueño. Por eso se
    // recorta AQUÍ, en el servidor, y no solo en pantalla: así el apellido
    // nunca llega al navegador de quien tenga el enlace.
    if (action === "ticket") {
      ...
      return NextResponse.json({
        ticket: { ...t, client_name: String(t.client_name || "").trim().split(/\s+/)[0] || "" },
      });
    }
