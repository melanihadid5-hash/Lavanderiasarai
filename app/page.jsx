...
// Construye la fecha exacta EN EL DISPOSITIVO del personal. Es importante
// hacerlo aquí y no en el servidor: Vercel corre en UTC, así que si el
// servidor interpretara "8am" guardaría las 8:00 UTC (2:00 a.m. en México).
function buildEstimateISO(hour, nextDay) {
  const d = new Date();
    ...
    <div style={{ maxWidth: 440, margin: "0 auto", width: "100%" }}>
      ...
      <div style={{ ...card, padding: 28 }}>
        ...
        <p style={{ fontSize: 12, color: MUTED, marginTop: 14, marginBottom: 0 }}>
          Al recoger tu ropa te pediremos tu apellido para confirmar que la
          entregamos a la persona correcta.
        </p>
  ...
  return (
    <div style={{ ...card, padding: 20, marginBottom: 16 }}>
      ...
      {matches.map((c) => (
        <button
          key={c.id}
          onClick={() => run({ clientId: c.id, hour, nextDay, estimateAt: buildEstimateISO(hour, nextDay) })}
          disabled={busy}
          style={{
            width: "100%",
            display: "flex",
      ...
      {canCreate && (
        <div style={{ border: `1px solid ${ACCENT}`, background: ACCENT_SOFT, borderRadius: 12, padding: 12, marginTop: 10 }}>
          ...
          <Button
            onClick={() =>
              run({
                newClient: { name: query.trim(), phone },
                hour,
                nextDay,
                estimateAt: buildEstimateISO(hour, nextDay),
              })
            }
            busy={busy}
          >
