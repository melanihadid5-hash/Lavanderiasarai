...
// Sugiere 3 horas después. Si ya no cabe antes del cierre, pasa a mañana.
function suggestEstimate() {
  const c = new Date().getHours() + 3;
      ...
      <div style={{ background: BG, borderRadius: 12, padding: 12, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => setNextDay(false)} style={chip(!nextDay)}>Hoy</button>
            <button onClick={() => setNextDay(true)} style={chip(nextDay)}>Mañana</button>
          </div>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {HOUR_OPTIONS.map((h) => (
            <button key={h} onClick={() => setHour(h)} style={chip(hour === h)}>
